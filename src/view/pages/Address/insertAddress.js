			...
			alert:
				...
				>
					<SCLAlertButton theme="success" onPress={() => {
						...
						const screen = this.props.navigation.getParam('screen', 'default')
						if (screen === "home")
	...
	async handleClick() {
		if (this.state.dataInfos.cepState === 'has-success' && this.state.dataInfos.addressState === 'has-success'
			...
			try {
				...
				await axios.post(FIREBASE_DATABASE_URL + "/app/accounts/" + userId + "/adresses.json", data)
					.then(res => {
						this.successAlert("MyPets", "Endereço inserido com sucesso!")
					})
			...
			<ScrollView contentContainerStyle={STYLE.container}>
				...
				{/* <Input
					...
					onChangeText={(value) => this.setState({
						dataInfos:
							Util.cepValidation(value, this.state.dataInfos)
					})}
				/> */}
				<TextInputMask
